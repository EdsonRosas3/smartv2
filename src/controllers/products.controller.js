...
export const updateProductById = async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: "Producto ha sido actualizado",product});
  } catch (error) {
    return res.status(500).json(error);
  }
};
...
export const deleteProductById = async (req, res) => {
  try {
    const publicnames = await Image.findAll({ 
      where:{product_id:req.params.productId},attributes:['public_name']
    });
    ...
    return res.status(200).json({ message: "Producto ha sido eliminado",deleteProduct });
  } catch (error) {
    return res.status(500).json(error);
  }
};
...
export const updateDefaultImage = async (req, res)=>{
  try {
    ...
    return res.status(201).json({ message: "La imagen principal se actualizo",updateProduct });
  } catch (error) {
    return res.status(500).json(error);
  }
}
